...
def _fwd_kernel(
    ...
    V_block_ptr = V + v_offset + tl.arange(0, e)[None, :]
    # O_block_ptr = Out + o_offset + e_offset + tl.arange(0, BLOCK_MODEL)[None, :]
    O_block_ptr = Out + o_offset + tl.arange(0, e)[None, :]
    # O_debug_block_ptr = Out_debug + o_offset + e_offset + tl.arange(0, BLOCK_MODEL)[None, :] # debug dump
    O_debug_block_ptr = Out_debug + o_offset + tl.arange(0, e)[None, :] # debug dump
    S_block_ptr = S + off_h
    ...
    kv = tl.full([d, d], value=1.0, dtype=tl.float32) # [d,d] or [d,e]
    offs_kv_m = tl.arange(0, d)[:, None]   # [d, 1]
    offs_kv_n = tl.arange(0, d)[None, :]   # [1, d]
    KV0_block_ptr = kv0_debug + kv_offset + offs_kv_m * d + offs_kv_n
    KV1_block_ptr = kv1_debug + kv_offset + offs_kv_m * d + offs_kv_n
    # import pdb
    # pdb.set_trace()
    ...
    for i in range(NUM_BLOCK):
        ...
        o_intra = tl.dot(qk, v)                # qk 64x64, v 64x128 -> o_intra 64x128
        o_inter = tl.dot(q, kv) * q_decay      # q 64x128, kv 128x128 -> 64x128, q_decay 64x1
        ...
        # pdb.set_trace()
        if i == 0:
            tl.store(KV0_block_ptr, kv)
        elif i == 1:
            tl.store(KV1_block_ptr, kv)
        off_block += BLOCK
...
lightning_attn2_naive = LightningAttention2.apply